...
Fetch the logged-in account's passport info (`/passport/web/account/info/`). Needs a real login session — pass it via `SESSIONID` or `COOKIE_OVERRIDE`.

```bash
SESSIONID="your_tiktok_sessionid" ./examples/passport-info.sh
```
